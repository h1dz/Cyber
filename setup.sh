#!/usr/bin/env bash
cd /$HOME/
sudo rm -rf Documents/ Videos/ Pictures/ Music/ Templates/ Public/ 2>/dev/null
sudo mkdir Tools
mkdir misc
mkdir home
...
sudo msfconsole -q -x "msfdb init; exit"
sudo wpscan --update
...
sudo wget https://wordlists-cdn.assetnote.io/data/automated/httparchive_js_2023_08_28.txt -O js.txt
...
sudo wget https://github.com/jpillora/chisel/releases/download/v1.7.7/chisel_1.7.7_linux_amd64.gz -O chisel_linuxv177.gz; sudo gunzip -d chisel_linuxv177.gz; sudo chmod +x chisel_linuxv177
...
sudo wget https://github.com/antonioCoco/JuicyPotatoNG/releases/download/v1.1/JuicyPotatoNG.zip; sudo unzip JuicyPotatoNG.zip; sudo rm -rf JuicyPotatoNG.zip
...
sudo git clone https://github.com/GerbenJavado/LinkFinder.git; cd LinkFinder; pip install jsbeautifier; sudo python setup.py install
sudo git clone https://github.com/m4ll0k/SecretFinder.git; cd SecretFinder; pip install -r requirements.txt
sudo git clone https://github.com/Dionach/CMSmap; cd CMSmap; sudo pip3 install .
cd /$HOME/Tools/web
sudo git clone https://github.com/lc/gau.git; cd gau; cd cmd; cd gau; sudo go build; sudo mv gau /usr/local/bin/;
cd /$HOME/home/
go install github.com/tomnomnom/waybackurls@latest
wget https://nmap.org/dist/nmap-7.92-win32.zip; unzip nmap-7.92-win32.zip; rm -rf nmap-7.92-win32.zip; mv nmap-7.92/ nmap/; sudo cp -r /$HOME/home/nmap/ /home/kali/Tools/windows/
...
sudo git clone https://github.com/calebstewart/CVE-2021-1675; cd CVE-2021-1675; sudo mv CVE-2021-1675.ps1 print-nightmare.ps1; sudo mv print-nightmare.ps1 /home/kali/Tools/windows/; sudo mv nightmare-dll /home/kali/Tools/windows/; cd ..; sudo rm -rf CVE-2021-1675/
sudo cp /usr/share/nishang/Shells/Invoke-PowerShellTcp.ps1 /home/kali/Tools/windows/Invoke-PowerShellTcp.ps1
...
sudo mkdir nmap; cd nmap; sudo wget https://nmap.org/dist/nmap-7.92-win32.zip -O nmap.zip; sudo 7z e nmap.zip; cd ..
sudo wget https://github.com/itm4n/PrintSpoofer/releases/download/v1.0/PrintSpoofer64.exe
sudo git clone https://github.com/t3l3machus/hoaxshell; cd ./hoaxshell; sudo pip3 install -r requirements.txt; cd ..
...
echo "alias msf2='sudo msfconsole -q -x \"use exploit/multi/handler; set PAYLOAD windows/x64/meterpreter/reverse_tcp; set LHOST tun0; set LPORT 443; exploit\"'" >> /$HOME/.bash_aliases
...
echo "alias rmv='sudo rm -rf hash id_rsa exploit.py exploit.sh hash.txt user.txt pass.txt names.txt name.txt sharesSMB.txt smbResults ports3.txt ports2.txt ports1.txt ports0.txtnmap.txt masscan4.txt masscan3.txt masscan2.txt masscan1.txt Gobusterscan.txt GobusterExt_DUMP.txt Gobuster_DUMP.txt; rm *.log'" >> /home/kali/.bash_aliases
echo "alias gob='sudo /$HOME/home/go.sh $1'" >> /$HOME/.bash_aliases
echo "alias wwe='script -f -c \"bash\" ./\$(date +%s%N | sha256sum | base64 | head -c 6).log'" >> $HOME/.bash_aliases
...
echo "alias msf2='sudo msfconsole -q -x \"use exploit/multi/handler; set PAYLOAD windows/x64/meterpreter/reverse_tcp; set LHOST tun0; set LPORT 443; exploit\"'" >> /$HOME/.zshrc
...
echo "alias rmv='sudo rm -rf hash id_rsa exploit.py exploit.sh hash.txt user.txt pass.txt names.txt name.txt sharesSMB.txt smbResults ports3.txt ports2.txt ports1.txt ports0.txtnmap.txt masscan4.txt masscan3.txt masscan2.txt masscan1.txt Gobusterscan.txt GobusterExt_DUMP.txt Gobuster_DUMP.txt; rm *.log'" >> /home/kali/.zshrc
echo "alias gob='sudo /$HOME/home/go.sh $1'" >> /$HOME/.zshrc
echo "alias wwe='script -f -c \"bash\" ./\$(date +%s%N | sha256sum | base64 | head -c 6).log'" >> /$HOME/.zshrc
